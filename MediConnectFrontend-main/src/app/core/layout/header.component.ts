...
// Interface pour les notifications (si tu ne l'as pas déjà dans un modèle)
  ...
  template: `
    <header class="app-header">
      ...
      <div class="header-right">
        ...
        <!-- BOUTON LECTURE VOCALE GLOBALE -->
        <button class="voice-read-btn" (click)="readPage()" [class.reading]="isReadingPage" 
                [title]="isReadingPage ? 'Arrêter la lecture' : 'Lire la page' " aria-label="Lire la page">
          <svg *ngIf="!isReadingPage" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z"/>
          </svg>
          <svg *ngIf="isReadingPage" fill="currentColor" viewBox="0 0 24 24">
    ...
    <style>
      ...
      .voice-read-btn svg { width: 17px; height: 17px; }
      .voice-read-btn:hover { 
        background: #ede9e3; color: #0d9488;
        transform: translateY(-1px);
        box-shadow: 0 2px 6px rgba(13,148,136,.2);
      }
      
      .voice-read-btn.reading {
        background: #0d9488; color: #fff;
        box-shadow: 0 0 0 3px rgba(13, 148, 136, 0.2);
        animation: pulse-reading 1.5s ease-in-out infinite;
      }
      
      .voice-read-btn.reading:hover {
        background: #0f766e;
...
export class HeaderComponent implements OnInit {
  ...
  // ✅ NOTIFICATIONS STATIQUES POUR TEST
  mockNotifications: Notification[] = [
    ...
    },
    {
      id: '4',
      type: 'PINNED',
      message: 'Votre post a été épinglé par un administrateur',
      read: false,
    ...
    },
    {
      id: '5',
      type: 'BADGE',
      message: 'Félicitations ! Vous avez obtenu le badge "Contributeur"',
      read: true,
  ...
  constructor(
    ...
    this.ttsService.isSpeaking$.subscribe(speaking => {
      this.isReadingPage = speaking;
    });
  }

  ngOnInit(): void {
    // Au lieu d'appeler le vrai service, on utilise les mock
  ...
  // ✅ Charge les notifications statiques
  ...
  markAsRead(notification: Notification, event: Event) {
    ...
    // Ici tu pourrais appeler le vrai service plus tard
  ...
  // Lit le contenu de la page actuelle - tous les éléments
  readPage() {
    ...
    const textToRead = this.extractPageContent();
    
    if (textToRead.trim().length > 0) {
      this.ttsService.speak(textToRead, 1, 1, 1);
    }
  }
  ...
  private extractPageContent(): string {
    ...
    selectors.forEach(selector => {
      ...
      elements.forEach((el: Element) => {
        ...
        if (text && text.length > 0 && !this.isCommonPhraseOnly(text)) {
          // Éviter les doublons
          if (!textParts.includes(text) && !this.isInPreviousText(text, textParts)) {
            textParts.push(text);
          }
        }
      });
    ...
    // Si aucun contenu trouvé, essayer une approche générale
  ...
  private isCommonPhraseOnly(text: string): boolean {
    const commonPhrases = [
      'plus',
      'moins',
      'ok',
    ...
    return commonPhrases.some(phrase => lower === phrase);
  }

  private isInPreviousText(text: string, textParts: string[]): boolean {
    return textParts.some(part => part.includes(text) || text.includes(part));
  }
